...
"""
(Gaussian) denoising convolutional auto-encoder with tied weights on MNIST
...
def main():
    ...
    ex  = np.array([mnist.train.images[0,:]])
    sig = 0.25
    ex_nz = ex + sig*np.random.standard_normal(ex.shape)
    
    for i in range(NUM_STEPS):
        batch_x,_ = mnist.train.next_batch(BATCH_SIZE)
        batch_x  = np.array(batch_x)
        batch_nz = batch_x+sig*np.random.standard_normal(batch_x.shape)
        train_step.run(feed_dict={x: batch_nz, y_: batch_x})
        if i%NUM_STEPS_OUT == 0:         
            mse_batch = mse.eval(feed_dict={x: batch_nz, y_: batch_x})
            print("Batch reconstruction MSE (step %d): %g" % (i, mse_batch))
            
            ex_recon = h_pool4.eval(feed_dict={x: ex_nz})

            plt.subplot(1,3,1)
            plt.imshow(np.reshape(ex, (28,28)))
            plt.subplot(1,3,2)
            plt.imshow(np.reshape(ex_nz, (28,28)))
            plt.subplot(1,3,3)
